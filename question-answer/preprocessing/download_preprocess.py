""" adapted from CS224n """
# Downloading the SQuAd v1.1 version 
# save into ./data/squad/
...
if __name__ == '__main__':
    ...
    parser.add_argument("--glove_source", choices = ['wiki', 'crawl_ci', 'crawl_cs'],
    ...
    print("Storing Datasets in {}".format(data_prefix))

    if not os.path.exists(data_prefix):
        os.makedirs(data_prefix)
    if args.glove_source == 'wiki':
        glove_zip = maybe_download(glove_base_url, glove_filename, data_prefix, 862182613)
    else:
        glove_zip = maybe_download(glove_base_url, glove_filename, data_prefix)
        
    # zip-out
    glove_zip_ref = zipfile.ZipFile(os.path.join(data_prefix, glove_filename), 'r')
    glove_zip_ref.extractall(data_prefix)
    glove_zip_ref.close()


    # original raw data
    download_prefix = os.path.join(data_prefix, "download")
